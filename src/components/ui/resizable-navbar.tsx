...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      /* Fix 2 — pre-hydration flash prevention.
         The `style` prop is included in the SSR HTML output, so the navbar
         renders with correct padding and width on first paint — before any
         JavaScript runs or Framer Motion hydrates.  This matches the
         visible=false animate values exactly, so there is no layout jump. */
      style={{
        paddingLeft: "185px",
        paddingRight: "185px",
        paddingTop: "40px",
      ...
      animate={{
        ...
        borderRadius: "0px",
        backgroundColor: visible ? "var(--navbar-scrolled-bg)" : "transparent",
      }}
      ...
      {items.map((item, idx) => (
        <a
          ...
          style={{
            ...
            fontWeight: 400,
            /* Fix 1b — rest color: --nav-label-rest-color is #F0EFE9 in dark theme */
            color: hovered === idx ? "var(--text-primary)" : "var(--nav-label-rest-color)",
          }}
        >
          {hovered === idx && (
            <motion.div
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-[var(--navbar-scrolled-bg)]",
        className,
      )}
    >
      {children}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <IconX className="cursor-pointer" style={{ color: "var(--text-primary)" }} onClick={onClick} />
  ) : (
    <IconMenu2 className="cursor-pointer" style={{ color: "var(--text-primary)" }} onClick={onClick} />
  );
};
